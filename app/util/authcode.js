...
// const { encode, decode } = require('@airx/authcode');
// encode('test','key')
...
const authcode = (str, key, operation = 'ENCODE', expiry = 0) => {
  ...
  if (operation === 'DECODE') {
    s = strbuf.toString();
    if (
      (s.substr(0, 10) === '0'.repeat(10) || s.substr(0, 10) - (0, getTimestamp)() > 0) &&
      s.substr(10, 16) === (0, md5)(s.substr(26) + keyb).substr(0, 16)
    ) {
      s = s.substr(26);
    } else {
      s = '';
