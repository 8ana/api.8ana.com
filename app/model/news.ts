...
export default app => {
  ...
  const News = model.define('news', newsSchema);

  News.query = async ({ attributes, pageSize = 10, pageNo = 1, order = ['created_at', 'DESC'] }) => {
    const condition = {
      attributes,
      order: [order],
  ...
  News.get = async (params, attributes = ['id', 'name', 'pic']) => {
    const condition = {
      attributes,
      where: {},
    };
  ...
  // 添加
  News.add = async params => {
    const result = await News.create(params);
    return result;
  };
  // 更新
  News.edit = async params => {
    const { id } = params;
