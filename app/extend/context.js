...
module.exports = {
  ...
  async hits({ arr, name, model }) {
    ...
    const last =
      typeof arr[lasttime] === 'number' && String(arr[lasttime]).length === 10 ? arr[lasttime] * 1000 : dayjs(arr[lasttime]).valueOf();
    const rid = `${name}_id`;
    const now = new Date();
    ...
    await this.app.model[model].update(h, { where: { [rid]: arr[rid] } });
    delete h[rid];
    delete h[lasttime];
    return { ...h };
  },
